...
import com.f.use.printdemo.print.LabelConnEvent;
...
//            queryCommand();
    ...
    /**
     * todo 2 查询当前连接打印机所使用打印机指令（ESC（EscCommand.java）、TSC（LabelCommand.java））
     */
    ...
    // todo 3 读取打印机返回信息
    private class PrinterReader extends Thread {
    ...
    private void queryPrinterCommand() {
        ThreadPool.getInstantiation().addTask(new Runnable() {
            ...
            public void run() {
                ...
                }), 500, TimeUnit.MILLISECONDS);// todo 2000->500->200->500
            }
        });
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = PrintApplication.getContext().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        // todo 设置当前打印机模式为ESC模式 - 小票
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                PrintApplication.getContext().sendBroadcast(intent);
                            } else if (result == 1) {//查询打印机实时状态
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " " + PrintApplication.getContext().getString(R.string.str_printer_out_of_paper);
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " " + PrintApplication.getContext().getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " " + PrintApplication.getContext().getString(R.string.str_printer_error);
                                }
                                System.out.println(PrintApplication.getContext().getString(R.string.str_state) + status);
                               Utils.toast(PrintApplication.getContext(), status);
                            }
                        }
                    } else if (sendCommand == tsc) {
                        //todo 设置当前打印机模式为TSC模式
                        if (currentPrinterCommand == null) {
                            currentPrinterCommand = PrinterCommand.TSC;
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                        } else {
...
//                                System.out.println(LogisticsApplication.getContext().getString(R.string.str_state) + status);
                                Utils.toast(PrintApplication.getContext(), status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
//                                LogisticsApplication.getContext().sendBroadcast(intent);
                                LabelConnEvent event = new LabelConnEvent();
                                event.intent = intent;
